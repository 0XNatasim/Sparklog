...
1. Cost reports combine draft, submitted, approved, pending, rejected, and current-rate data into one total.
2. Job creation and submission lack a durable idempotency key, and direct API clients can insert an incomplete job directly as submitted.
3. Opening the employee-management panel can overwrite compensation fields for many employees.
4. Attachment, claim, notification, approval, and export workflows are multi-step and can leave partial or ambiguous states.
...
### 1.2 Resolved — Skipped single-job approval could become approved without export
...
The client-side fallback update has been removed. A single-job approval is now considered successful only when the Edge Function explicitly reports exactly one exported row. Skipped, changed, or concurrently claimed results do not trigger any browser-side job mutation; the dashboard reloads authoritative state and tells the manager that no approval was applied.

Unit coverage verifies that skipped and `already_claimed` responses cannot be interpreted as successful exports. Typed per-job server outcomes remain a useful follow-up for giving managers a more specific reason, while the safety defect itself is contained.
...
| Manager | Stale single-job approval | Resolved: skipped/changed jobs are reloaded without a client-side approval write | State-change conflict and re-review | Client safety fix complete; typed server reason remains a follow-up |
...
1. Add explicit overnight confirmation and block malformed, zero, excessive, and overlapping jobs at submission and approval.
2. Add durable per-submission idempotency.
3. Separate Costing by status and stop using current rates for historical approved work.
4. Remove compensation writes from EmployeesPanel loading.
5. Make job/evidence/expense/notification submission atomic or explicitly recoverable.
6. Persist immutable approval snapshots and durable export attempts.
7. Repair actor attribution for service-role approval audits.
8. Replace broad manager updates with server-enforced state transitions.
9. Remove client-supplied role from profile synchronization.
10. Add cursor pagination and query bounds to manager and employee history surfaces.
11. Implement an identity-bound IndexedDB offline queue with attachment support.
12. Replace hardcoded NAS identities and direct bulk browser access.
13. Add error boundaries, route splitting, dependency gates, and hostile-condition integration tests.
